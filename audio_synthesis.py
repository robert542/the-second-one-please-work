...
def synthesize_sound(base_freq, sample_rate, max_harmonic, volume, add_fall_off):
    ...
    for i in range(1, 2 * max_harmonic, 2):  # Generating odd multiples up to the 2n-1 harmonic
        freq = base_freq * i
        if i != 1:
            amp = (0.5 / (10 ** (i // 2))) * volume  # More intense drop-off for harmonics
        else:
...
def main():
    ...
    if uploaded_file is not None:
        ...
        data = data.astype(np.float32) / np.iinfo(data.dtype).max  # Normalize audio data
    ...
    base_freq = st.slider("Base Frequency", 80.0, 130.0, 100.0,0.01)  # Adjusted base frequency
    max_harmonic = st.slider("Max Harmonic (n)", 1, 40, 3)
